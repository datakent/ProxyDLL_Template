...
void sub_fnLoad()
{
    ...
    {
        MessageBoxA(NULL, "DLL FN bağlantı hatası.", "hata", 0);
        return;
    }
...
bool WriteOpCodesToExe(DWORD fileOffset, const BYTE* opCodes, size_t opCodeSize) {
    ...
    // 3. Section header'larını bul
    PIMAGE_SECTION_HEADER pSection = IMAGE_FIRST_SECTION(pNtHeaders);
    bool found = false;

    for (WORD i = 0; i < pNtHeaders->FileHeader.NumberOfSections; i++, pSection++) {
        // 4. Fiziksel offset'in hangi section'da olduğunu bul
    ...
    //Hedef dosya ofseti hiçbir bölümde bulunamadı!
    if (!found) {
        return false;
    }
